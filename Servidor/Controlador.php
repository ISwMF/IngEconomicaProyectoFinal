...
if (isset($_GET['NumCuota'])) {
    ...
    echo "<input type=\"text\" name=\"IP\" id=\"IP\" class=\"form-control\" ng-model=\"IP\" value=\"",number_format($IP, 4),"\"  readonly>";
    echo "</div>";
    ...
    echo "<tr><td>", $_GET['NumCuota'],  "</td><td>",$FechaRetardo->format('d/m/Y'),"</td><td>",number_format($_GET['CuotaFija'], 2, ',', '.'),"</td><td>",number_format($Mora, 2, ',', '.'),"</td><td>",number_format(($_GET['CuotaFija'] + $Mora), 2, ',', '.'),"</td></tr>";
    echo "</table>";
    echo "<div class=\"ButtonPFD\"><button type=\"button\" name=\"button\" class=\"btn btn-success btn-block\" onclick=\"createPDF('t')\">Descargar PDF con mora</button></div>";
