        ...
        for ($i=0; $i <count($tabla) ; $i++) {
            ...
            for ($j=0; $j <count($tabla[$i]) ; $j++) {
                if (strpos($tabla[$i][$j], '/')) {
                    ...
                    echo "<td>",number_format($tabla[$i][$j], 2, ',', '.'),"</td>";
                }
            }
            echo "</tr>";
        }
        echo "</table>";
        echo "<div class=\"ButtonPFD\"><button type=\"button\" name=\"button\" class=\"btn btn-success btn-block\" onclick=\"createPDF('f')\">Descargar PDF sin mora</button></div>";
